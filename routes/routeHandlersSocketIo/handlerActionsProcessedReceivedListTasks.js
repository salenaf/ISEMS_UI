...
module.exports.receivedListTasksDownloadFiles = function(socketIo, data, sessionId){
    ...
    debug("===================");
    debug(tmpModuleNetworkInteraction);
    ...
    debug("********* before **********");
    debug(tasksDownloadFiles.listTasksDownloadFiles);
    debug("*******************");
    ...
    if(data.options.p.ccn === 1){
        let newTaskList = require("../../libs/helpers/helpersFunc").modifyListFoundTasks(data.options.slft.slice(0, MAX_CHUNK_SIZE));
        ...
        socketIo.emit("module NI API", { 
            ...
            "options": {
                /*p: {
                    cs: data.options.p.cs, //размер части
                    cn: data.options.p.cn, //всего частей
                    ccn: data.options.p.ccn, //номер текущей части
                },*/
                p: {
