...
module.exports = function(req, res, objHeader, socketIo) {
    async.parallel({
        ...
        if (err) {
            ...
            let readStatus = result.userGroupPermissions.group_settings.management_geoip.element_settings.read.status;
            if (readStatus === false) return res.render('403');

            res.render('menu/settings/setting_geoip', {
                header: objHeader,
                userGroupPermissions: result.userGroupPermissions.group_settings.management_geoip.element_settings,
